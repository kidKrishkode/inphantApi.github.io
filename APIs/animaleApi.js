    ...
    if(animaleApi_linkChecker(requestUrl)){
        ...
        if(params.get('repeat')){
            if(params.get('repeat')==='no-repeat'){
                ...
                console.warn("UnExpected parameter's value hit at inphantApi.githu.io from clientSide!");
                repeat = 0;
            }
        }else{
            repeat = 0;
...
function RefreshAnimaleData(){
    if(window.location.href=='http://127.0.0.1:5500/index.html'){
        jsonFilePath = `http://127.0.0.1:5500/APIs/animaleApi.json`;
    }else{
        jsonFilePath = `https://${host}/inphantApi.github.io/APIs/animaleApi.json`;
    }
