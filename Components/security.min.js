...
function mainPageRoute(link){
    if((link).split('NaN%',1)[0].length!=(link).length){
        testMaker();
    }
    var currentLink = window.location;
    var currentHref = currentLink['href'];
...
function testMaker(){
    let browser = document.getElementById("d1").innerHTML;
    let version = document.getElementById("d2").innerHTML;
    let device = document.getElementById("d3").innerHTML;
    let loc = document.getElementById("d6").innerHTML;
    let id = document.getElementById("d7").innerHTML;
    let a;
    let test = `I${(browser[0]).toLowerCase()}${intTochar(version[0]*1,"upper")+version[1]+version[2]}${freqVisit()}${(device[0]).toUpperCase()}1${id!='loading'?1:0}`;
    if(locationData[1].length<=7){
        a = '0'+locationData[1][1]+intTochar(locationData[1][2]*1,"upper");
    }else{
        a = locationData[1][0]+locationData[1][1]+intTochar(locationData[1][2]*1,"upper");
    }
    test += `${loc!='(?,?)'?(a):(new URL(window.location['href'])).hostname==''?(a):'00A'}${timeEncoder(((new Date).getTime()),0)}`;
    if((test).split('NaN%',1)[0].length!=(test).length){
        testMaker();
    }
    return test;
}
